...
class NseHistoricalBenchmarkFetcher:
    ...
    async def start(self) -> None:
        ...
        try:
            ...
            # against a real report landing page instead (mirrors nse_flows.py /
            # nse_financial_results.py, which do not hit NSE_HOME and do not 403 in CI).
            response = await self._client.get(NSE_INDEX_HISTORY_PAGE)
    ...
    async def fetch(
        ...
        *,
        ...
        if landing_page != NSE_INDEX_HISTORY_PAGE:
            # start() already warmed against the index-history page; for any other
            # benchmark, warm the session against its own landing page too so the
            # first API request carries a matching Referer/cookie pair instead of
            # relying solely on the 401/403 fallback below.
            await self._refresh_session(landing_page)
