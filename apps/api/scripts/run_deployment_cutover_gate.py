...
def _load_json(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(payload, dict):
        raise TypeError("deployment-cutover evidence file must contain a JSON object")
    secret_paths = _secret_like_paths(payload)
    if secret_paths:
        raise ValueError(
            "deployment-cutover evidence must not contain secret-like keys: "
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Run the fail-closed Phase 47-50 deployment-cutover gate from "
            "externally collected non-secret evidence."
        )
    )
    parser.add_argument(
        "--evidence-json",
        default=os.getenv("DEPLOYMENT_CUTOVER_EVIDENCE_JSON", ""),
    ...
    if args.plan_only:
        print(json.dumps(required_deployment_cutover_contract(), indent=2, sort_keys=True))
        return 0
            ...
            json.dumps(
                {
                    "ready": False,
                    "status": "failed",
                    "error": "DEPLOYMENT_CUTOVER_EVIDENCE_JSON or --evidence-json is required",
