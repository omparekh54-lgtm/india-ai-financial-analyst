...
def test_market_context_bootstrap_has_fail_fast_dependency_order() -> None:
    ...
    assert "backfill_nse_benchmarks.py" in commands[0][1][1]
    assert "sync_india_vix_macro.py" in commands[1][1][1]
    assert "backfill_nse_fii_dii.py" in commands[2][1][1]
    assert "backfill_rbi_10y.py" in commands[3][1][1]
...
def test_market_context_bootstrap_propagates_freshness_limits() -> None:
    ...
    assert commands[0][1][-2:] == ["--min-rows", "60"]
    assert commands[1][1][-2:] == ["--max-age-days", "2"]
    assert commands[2][1][-2:] == ["--max-age-days", "3"]
    assert commands[3][1][-2:] == ["--max-age-days", "30"]
