...
def test_release_gate_runs_every_required_stage_in_order() -> None:
    ...
    assert "run_agent_readiness_gate.py" in stages[1].command[1]
    assert "run_deployment_smoke.py" in stages[2].command[1]
    assert "--require-corpus-ready" in stages[2].command
    assert "verify_auth_isolation.py" in stages[3].command[1]
    assert "run_load_probe.py" in stages[4].command[1]


def test_release_gate_never_places_access_tokens_on_cli() -> None:
...
def test_release_gate_requires_https_except_explicit_localhost() -> None:
    ...
    assert "--allow-http-localhost" in stages[3].command
